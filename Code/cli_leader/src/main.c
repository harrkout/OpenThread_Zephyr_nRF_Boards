...
static void udp_send_thread_func(void *p1, void *p2, void *p3)
{
    ...
    while (1) {
        ...
        for (int i = 0; i < 6; i++) {
            buffer[i] = rand() % 100;
            offset += snprintf(msg + offset, sizeof(msg) - offset, (i == 0) ? "%d" : ",%d", buffer[i]);
            if (offset >= sizeof(msg) - 1) break;
        }
        msg[offset] = '\0';

        snprintf(command, sizeof(command), "ot udp send fe80::b866:a487:ef8a:5e85 1234 %s", msg);
        shell_fprintf(shell, SHELL_NORMAL, "\nSending: %s\n", msg);

        int err = shell_execute_cmd(shell, command);
        if (err != 0) shell_fprintf(shell, SHELL_ERROR, "Command failed: %d\n", err);
...
void main(void) {
   ...
   // Create the UDP send thread
    k_thread_create(&udp_send_thread, udp_send_stack, 
                    K_THREAD_STACK_SIZEOF(udp_send_stack),
                    udp_send_thread_func, NULL, NULL, NULL,
                    THREAD_PRIORITY, 0, K_NO_WAIT);
